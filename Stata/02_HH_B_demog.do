...
g byte christianHoh = (hh_b23 == 3 & hoh == 1)
la var christianHoh "head of household identifies as Christian"
...
* Calculate demographics for controls
g byte hoh = hh_b04 == 1
la var hoh "head of household"
...
g agehead = hh_b05a if hoh == 1
la var agehead "age of the head of household"

g  genderhead = hh_b03 if hoh == 1
la var genderhead "gender of hoh"
la def sex 1 "male" 2 "female"
la val genderhead sex

* ---- Relationship status
g byte marriedHoh = inlist(hh_b24, 1, 2) & hoh == 1
g byte marriedHohm = inlist(hh_b24, 1) & hoh == 1
g byte marriedHohp = inlist(hh_b24, 2) & hoh == 1
g byte nonmarriedHoh = (inlist(hh_b24, 3, 4, 5, 6) == 1) & hoh == 1
g byte widowFemhead = (femhead == 1 & hh_b24 == 5)
g byte nonmarriedFemhead = (femhead == 1 & inlist(hh_b24, 3, 4, 5, 6))

la var marriedHoh "married head of household (any)"
la var marriedHohm "monogamous married hoh"
la var marriedHohp "polygamous married hoh"
la var nonmarriedHoh "non-married (never marry, divorce, spearated, widowed)"
la var widowFemhead "widowed female hoh"
la var nonmarriedFemhead "Non-married female hoh"
...
egen `hohrelig' = max(religHoh), by(case_id)
egen `sprelig' = max(religSpouse), by(case_id)
g byte mxreligHH = `hohrelig' != `sprelig' & !missing(`hohrelig', `sprelig')

la var religHoh "religion of hoh"
la var religSpouse "religion of spouse"
la var mxreligHH "mixed-religion household"


* ---- Household demographics
* Gender ratio for households
g byte male = hh_b03 == 1
g byte female = hh_b03 == 2
la var male "male hh members"
la var female "female hh members"
...
egen msize = total(male), by(case_id)
egen fsize = total(female), by(case_id)
la var msize "number males in hh"
la var fsize "number females in hh"

* Create gender ratio variable
g gendMix = msize/fsize
recode gendMix (. = 0) if fsize == 0
la var gendMix "Ratio of males to females (1 = 1:1)"

* Flag observations under 60 months (5 years) for chronic malnutrition work later
g byte und5tmp = (hh_b05a <= 5)

* ---- Age group chunks for ratios and composition
egen youthtmp = cut(hh_b05a), at(0, 5, 10, 12, 15, 18, 25, 31, 36, 60, 65, 100) icodes
...
* Create binaries for demographic categories
g byte under5tmp = inlist(youthtmp, 0)
g byte under15tmp = inlist(youthtmp, 1, 2, 3)
g byte under24tmp = inlist(youthtmp, 1, 2, 3, 4, 5)
g byte youth15to24tmp = inlist(youthtmp, 4, 5)
g byte youth18to30tmp = inlist(youthtmp, 5, 6)
g byte youth25to35tmp = inlist(youthtmp, 6, 7)
g byte over35under65tmp = inlist(youthtmp, 8, 9)
...
* Create total, male and female total at the hh level of each demographic
local demo under5 under15 under24 youth15to24 youth18to30 youth25to35 over35under65 over64
	...
	foreach x of local demo {
		...
		* repalce missing values w/ zeros
		replace `x' = 0 if `x' == .
		replace `x'm = 0 if `x'm == .
		replace `x'f = 0 if `x'f == .

		la var `x' "total hh members `x'"
		la var `x'm "total hh members `x'm"
		la var `x'f "total hh member `x'f"
		}
	*end

/* Create intl. household dependency ratio
# HH Dep ratio = [(# people 0-14 + those 65+)]/[# people aged 15 - 64] * 100 */
g byte numDepRatio = inlist(youthtmp, 4, 5, 6, 7, 8, 9) != 1
g byte denomDepRatio = inlist(youthtmp, 4, 5, 6, 7, 8, 9) == 1
egen totNumDepRatio = total(numDepRatio), by(case_id)
egen totDenomDepRatio = total(denomDepRatio), by(case_id)

assert hhsize == (totNumDepRatio + totDenomDepRatio)

g depRatio = (totNumDepRatio / totDenomDepRatio) * 100 if totDenomDepRatio != .
la var depRatio "Dependecy ratio"


* ---- Household labor shares: those aged 12 - 60 considered laborers
g byte hhLabort = inlist(youthtmp, 3, 4, 5, 6, 7, 8) == 1
egen hhlabor = total(hhLabort), by(case_id)
la var hhlabor "hh labor age>11 & < 60"
...
* Male and female labor Shares
g mlaborShare = mlabor/hhlabor
recode mlaborShare (. = 0) if hhlabor == 0
la var mlaborShare "share of working age males in hh"

g flaborShare = 1 - mlaborShare
la var flaborShare "share of working age females in hh"

* ---- Demographic composition in households
*--------------------------------------
* % of hh females aged 20-34 & 35 - 59
g byte fem20_34tmp = (hh_b05a >= 20 & hh_b05a < 35) & (female == 1)
g byte fem35_59tmp = (hh_b05a >=35 & hh_b05a <60) & (female == 1)
...
g byte ChewaSpokenHoh = (hh_b22 == 1)

* ---- father present
g byte fatherPresent = (hh_b16a == 1)
g byte motherPresent = (hh_b19a == 1)
la var fatherPresent "father present in hh"
la var motherPresent "mother present in hh"

* Mobile phones in hh
g ownsMobile = (hh_b04a == 1)
g ownsMobileHoh = (hh_b04a == 1) & hoh == 1
g ownsMobileSpouse = (hh_b04a == 1) & spouse == 1
egen totMobiles = total(ownsMobile), by(case_id)

la var ownsMobile "household members own mobile"
la var ownsMobileHoh "hoh owns mobile"
la var ownsMobileSpouse "spouse owns mobile"
la var totMobiles "totle working cell phones in hh"


* ---- collapse down to household level
qui include "$pathdo/copylabels.do"

ds(hh_b*  case_id HHID PID), not
collapse (max) `r(varlist)', by(case_id)
